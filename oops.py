...
"""
the oops command, something i've wanted for eons since i was a
little boy. i kid you not.

basically it lets you replace the occurance of a mistake in the previous
command. it works by assuming that it was a minor spelling mistake and
finding the first closest match in your command.

usage:
   $ rmacs ~/bin/oops.py
   ksh: rmacs: not found
   $ oops emacs
   [emacs opens up and voila ...]

bugs and such:
- not extensively tested at all (ksh, bash, csh could use more testing)
- it doesn't leave a corrected mark in your history file
"""

"""
bktree.py, by leonardo maffi, V.1.0, Apr 18 2008
...
If you need a faster version (about 4.5 times faster) look for the same functions
inside my D libs:
http://www.fantascienza.net/leonardo/so/libs_d.zip

Open source code, Python licence.
"""
...
if __name__ == "__originmain__":
    import doctest
    doctest.testmod()
    print "Tests finished."

    # You can find one list of words here:
    # http://www.fantascienza.net/leonardo/so/text_spacer.zip
    #words = iter(file("wordlist2.txt").read().split())
...
def oops(history):
    ...
    l = cmd.split()
    try:
        # handle csh and derivatives, they store history as n HH:MM cmd
        time.strptime(l[1], '%H:%M')
        l = l[2:]
    except ValueError: l = l[1:]
    if l[0] in ('oops', 'history'):
        oops(history[:-1])
    # print 'newcmd is %s' % newcmd
    # print 'cmd is %s' % l
    D = []
    for i in l:
        e = editDistance(newcmd, i)
        D.append((e, i))
        # print e
    subs_min = min([ x[0] for x in D ])
    N = []
    for score, cmd in D:
        if score == subs_min:
            N.append((score, newcmd))
        else: N.append((score, cmd))
    # print D
    # print N
    os.system(' '.join([ x[1] for x in N]))

if __name__ == '__main__':
    history = [ x.strip() for x in open('/tmp/oops_history', 'r').readlines() ]
    oops(history[:-1])
